<template>
  <div style="position: relative; overflow: hidden; min-width: 950px;">
        ...
        <div>
          <div class="three-section__todo">
            <div class="three-section__phone">
              <img width="410" height="712"
                src="https://dpubstatic.udache.com/static/dpubimg/Vx5n_3YCtP/anli_pic_phone.png" alt="phone"
                loading="lazy" />
            </div>
            <div class="three-section__iframe">
              <iframe title="todo示例" scrolling="no" width="325" height="617"
                src="https://gift-static.hongyibo.com.cn/static/kfpub/3547/index.html" frameborder="0">
              </iframe>
            </div>
          </div>
        </div>
    ...
    <section class="section four-section">
      <div class="grow four-section__bg" :style="{ backgroundImage: `url(${frontmatter.fourSection.bg})` }">
      </div>
      <div class="four-section__inner">
        <div class="grow" style="position: relative;">
          <img width="412" :src="frontmatter.fourSection.img" alt="svg" loading="lazy" />
        </div>
        <div class="four-section__text grow">
          <div style="margin-left: 86px;">
            <div>
              <span class="dot-inner"></span>
              <p class="title">{{ frontmatter.fourSection.title }}</p>
              <p class="desc">
    ...
    <section class="section five-section">
      ...
      <div class="five-section__inner">
        ...
        <div class="grow" style="position: relative; display: flex; justify-content: flex-end;">
          <img width="412" :src="frontmatter.fiveSection.img" alt="svg" loading="lazy" />
        </div>
      </div>
      ...
      <div class="row seven-section__wrap">
        ...
        <div class="grow seven-section__center" :style="{ backgroundImage: `url(${frontmatter.sevenSection.bg})` }">
          <div class="seven-section_phone">
            <img width="213" height="433" src="https://dpubstatic.udache.com/static/dpubimg/Vx5n_3YCtP/anli_pic_phone.png"
...
import SwiperImg from '@theme/components/SwiperImg.vue'
import CodeList from '@theme/components/CodeList.vue'

// eslint-disable-next-line
interface Frontmatter extends Record<string, any> {
  threeSection: ThreeSection
  sixSection: Section
  sevenSection: Section
}
type ThreeSection = {
  title: string
  details: string
...
// eslint-disable-next-line
const frontmatter = useData().frontmatter as Record<string, any> as Frontmatter
const currentIndex = ref(0)
const list = computed(() => {
  let result = []
  let { details } = frontmatter.value.sixSection
  let current: Detail[] = []
  let i = 0
...
const dataList = computed(() => frontmatter.value.sevenSection.details)
const mvcList = computed(() => {
  return frontmatter.value.threeSection.list
})
const currentTitle = computed(() => dataList.value[currentIndex.value].title)
